                ...
                <div class="row p-2">
                  ...
                  <div class="col-md-2">
                    <datepicker class="form-control" v-model="NewVrf.date_from" style="width: 7rem"
                      inputFormat="dd/MM/yyyy" @update:model-value="setFromDateAdd" ref="dateFromPicker"
                      id="dateFromInput" />
                  ...
                  <div class="col-md-2">
                    <datepicker class="form-control" v-model="NewVrf.date_to" style="width: 7rem"
                      inputFormat="dd/MM/yyyy" @update:model-value="setToDateAdd" />
            ...
            <div class="modal-content">
              ...
              <div class="modal-body modalcustom">
                <div class="container modalcustom">
                  ...
                  <div class="row ms-4 ps-4 pe-3 me-3 pb-0 modalcustom scrollable-container">
                    <table class="table table-hover modalcustomtb">
                      <thead>
                        <tr>
                          <th scope="col" class="colwidth25">ชื่อ-นามสกุล</th>
                  ...
                  <div class="row p-0 pt-0 m-0">
                    <div class="col-12 ps-4 d-flex">
                      <div class="status-container">
                        <div class="status-steps">
                          <div v-for="(step, index) in steps" :key="step.status" class="step" :class="{
                            completed: approveStatus >= step.status,
                            active: approveStatus === step.status,
                          }">
                            <!-- วงกลม ✔ หรือว่าง -->
                            <div class="circle">
                              {{ approveStatus >= step.status ? "✔" : "" }}
                            </div>
                            <div class="label">{{ step.label }}</div>

                            <!-- เส้นเชื่อมระหว่างสถานะ -->
                            <div v-if="index <  steps.length - 1" class="line"
                              :class="{ completedLine: approveStatus >= step.status }"></div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div class="row modalcustom scrollable-container" style="margin-left: 0; padding-left: 8rem;">
                    <table class="table table-hover modalcustomtb mt-3" style="width: 40vw; margin: 0 auto 0 0;">
                      <thead>
                        <tr>
                          <th scope="col" class="text-start">สถานะการอนุมัติ</th>
                        </tr>
                      </thead>
                      <tbody>
                        <tr v-for="(data, index) in vrf_Existing.vrf_Existing_ApproveInfo" :key="index">
                          <td scope="col" style="white-space: nowrap; text-align: left;">
                            <div style="display: flex; align-items: flex-start; justify-content: flex-start;">
                              <input type="text" class="form-control"
                                :class="{ 'text-danger': vrf_Existing.vrf_Existing_ApproveInfo[index].approve_status_name.includes('รอ ') }"
                                style="width: 20rem; display: inline-block; border: none; background: transparent; box-shadow: none; outline: none; padding: 0;"
                                v-model="vrf_Existing.vrf_Existing_ApproveInfo[index].approve_status_name"
                                :disabled="!checkstatus_send_to_approve" />
                            </div>
                          </td>
                        </tr>
          ...
          <div class="modal-dialog modalcustom">
            <div class="modal-content">
              <div class="modal-header">
                ...
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"
                  @click="ClosemyModalUrgent"></button>
              </div>
              <div class="modal-body modalcustom">
                <div class="container modalcustom">
                  <!---Loop Manual Edit VRF------>
                  <div class="row ms-4 ps-4 pe-3 me-3 modalcustom scrollable-container">
                    <table class="table table-hover modalcustomtb">
                      ...
                      <tbody>
                        <!-------------------------------------existing vrf---------------------------------------->
                        <tr v-for="(data, index) in vrf_Existing.vrf_Existing_det" :key="index">
                          <td scope="col" class="colwidth25">
                            <div style="
                                display: flex;
                                align-items: center;
                                justify-content: center;
                              ">
                              <select class="form-select form-select-sm" v-model="vrf_Existing.vrf_Existing_det[index].prefix_id
                                " style="width: 7rem; margin-right: 0.5rem" @change="
                                  validateInput(
                                    'prefix_id',
                                    index,
                  ...
                  <div class="row p-2">
                    ...
                    <div class="col-md-2">
                      <datepicker class="form-control" v-model="vrf_urgent.date_from" style="width: 7rem"
                        inputFormat="dd/MM/yyyy" @update:model-value="setToDate('date_from_edit')"
                        :disabled="!checkstatus_send_to_approve" />
                      <p v-if="VRF_error.date_from && !vrf_urgent.date_from" class="error-message">
                        กรุณาเลือกข้อมูล
                      </p>
                    </div>
                    <div class="col-md-2 text-end">ถึงวันที่:</div>
                    <div class="col-md-2">
                      <datepicker class="form-control" v-model="vrf_urgent.date_to" style="width: 7rem"
                        inputFormat="dd/MM/yyyy" @update:model-value="setToDateAdd"
                        :disabled="!checkstatus_send_to_approve" />
                      <p v-if="VRF_error.date_to && !vrf_urgent.date_to" class="error-message">
                        กรุณาเลือกข้อมูล
                      </p>
                    </div>
                    <div class="col-md-2 text-end">
                  ...
                  <div class="row p-2">
                    ...
                    <div class="col-md-2">
                      ...
                      <p v-if="VRF_error.reason && !vrf_urgent.reason" class="error-message">
                        กรุณากรอกข้อมูล
                      </p>
                    </div>
                    <div class="col-md-2 text-end">ชื่อบริษัทผู้มาติดต่อ:</div>
                    <div class="col-md-2">
                      ...
                      <p v-if="VRF_error.contactor && !vrf_urgent.contactor" class="error-message">
                        กรุณากรอกข้อมูล
                      </p>
                    </div>
                    <div class="col-md-2 text-end">แนบไฟล์:</div>
                  ...
                  <div class="row p-2">
                    ...
                    <div class="col-md-2">
                      <!-- <input type="text" class="form-control" style="width:15rem;"
                        v-model="vrf_urgent.requestor_id" /> -->
                      <!----why can't set default selected-->
                      <!-- <VueMultiselect :options="data_ddl_edt.userlist"
                        class="form-select form-select-sm p-0" label="first_name" :style="{
                          width: '15rem'
                          , height: '0.5rem'
                        }" v-model="vrf_urgent.requestor_id" :select-label="null" :allow-empty="true"
                        :close-on-select="true" :value="user_id" track-by="user_id" placeholder="เลือก"
                        :deselectLabel=null>
                      </VueMultiselect> -->
                      ...
                      <p v-if="VRF_error.requestor && !vrf_urgent.requestor" class="error-message">
                        กรุณาเลือกข้อมูล
                      </p>
                    </div>
                    <div class="col-md-2 text-end">ตำแหน่งผู้ร้องขอ:</div>
                    <div class="col-md-2">
                      ...
                      <!-- <VueMultiselect name="requestor_position" id="requestor_position" :options="data_ddl.position"
                        class="form-select form-select-sm p-0" label="position" :style="{
                          width: '15rem'
                          , height: '0.5rem'
                        }" v-model="vrf_urgent.requestor_position_id" :select-label="null" :allow-empty="true"
                        :close-on-select="true" :value="id" track-by="id" placeholder="เลือก" :deselectLabel=null>
                      </VueMultiselect> -->
                      <p v-if="VRF_error.requestor_position_id &&
                    ...
                    <div class="col-md-2">
                      ...
                      <!-- <VueMultiselect name="requestor_dept" id="requestor_dept" :options="data_ddl.dept"
                        class="form-select form-select-sm p-0" label="department" :style="{
                          width: '15rem'
                          , height: '0.5rem'
                        }" v-model="vrf_urgent.requestor_dept_id" :select-label="null" :allow-empty="true"
                        :close-on-select="true" :value="id" track-by="id" placeholder="เลือก" :deselectLabel=null>
                      </VueMultiselect> -->
                      <p v-if="VRF_error.requestor_dept &&
                    ...
                    <div class="col-md-2">
                      ...
                      <!-- <VueMultiselect name="navigator" id="navigator" :options="data_ddl.navigator"
                        class="form-select form-select-sm p-0" label="fullname" :style="{
                          width: '15rem'
                          , height: '0.5rem'
                        }" v-model="vrf_urgent.navigator" :select-label="null" :allow-empty="true"
                        :close-on-select="true" :value="user_id" track-by="user_id" placeholder="เลือก"
                        :deselectLabel=null>
                      </VueMultiselect> -->
                      <p v-if="VRF_error.navigator_id && !vrf_urgent.navigator_id
                      " class="error-message">
                        กรุณาเลือกข้อมูล
                      </p>
                    </div>
              ...
              <div class="modal-footer pt-0 justify-content-center">
                <div class="align-top pt-1 d-flex justify-content-center">
                  ...
                  <button class="btn btn-danger" data-bs-dismiss="modal" type="reset" style="width: 4rem; height: 2rem"
                    id="ClosemyModalUrgent_" @click="ClosemyModalUrgent">
                    ยกเลิก
                  </button>
                </div>
              </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    //---code progress bar
    const approveStatus = ref(4); // ทดสอบสถานะ 1-5

    // const steps = [
    //   { status: 1, label: "สร้างรายการ" },
    //   { status: 2, label: "หัวหน้าอนุมัติ" },
    //   { status: 3, label: "หัวหน้า พท.มั่นคง อนุมัติ" },
    //   { status: 4, label: "NCC อนุมัติ" },
    //   { status: 5, label: "SU อนุมัติ / อนุมัติรายการ" },
    // ];
     const steps = ref([]);
    //---end code progress bar
    ...
    //-------------------end validateInput_vrfurgentcase
    ...
    const validateInput = (field, index, errorMessage) => {
      ...
      if (field === 'tbFullName') {
        ...
        // ตรวจสอบ dropdownlist ว่ามีค่าหรือไม่
        if (!rowData.value[index].ddlprefix || rowData.value[index].ddlprefix === 'เลือกคำนำหน้า') {
          rowData.value[index].errors = {
            ...rowData.value[index].errors,
    ...
    const ClosemyModalNew_ = () => {
      ...
      vrf_Existing.vrf_Existing_ApproveStatus = []
      vrf_Existing.errors = {}
      //------------------------------------------
      vrf_urgent.no = ''
      vrf_urgent.id = ''
      ...
      vrf_urgent.vrf_Existing_ApproveStatus = []
      vrf_urgent.errors = {}
      //---------------------------
    ...
    //
    //-----MeetingAreas
    const MeetingAreas_selectedItems = ref([]) //-----MeetingAreas
      ...
      columns: [
        ...
        },
        {
          ...
          display: function (row) {
            ...
            let type_modal_edit =
              row.req_urgentcase_by !== null
                ? 'ModalUrgentvrf' //? 'ModalEditUrgentCaseVrf'
                : 'ModalEditvrf'
            let wording_edit =
      ...
      Array.prototype.forEach.call(elements, function (element) {
        ...
        if (element.classList.contains('editvrf')) {
          element.addEventListener('click', async function () {
            ...
            // declare progress variable
             const progress  = reactive({
              approveStatus_: 1,
              MeetingAreas_selectedControlItems_length: 0
            })
            //---------------------------------get vrf header---------------------------------------------
              ...
              await axios
                ...
                .then(
                  (res) => {
                    ...
                    progress.approveStatus_ = obj[0].approve_status//---set approve status of progress bar
                  },
                  (res) => {
                    // error callback
                    console.log(res.data)
                ...
                .then(
                  (res) => {
                    ...
                    if (Array.isArray(obj)) {
                      ...
                      MeetingAreas_selectedItems.value = generalAreas
                      MeetingAreas_selectedControlItems.value = secureAreas
                      progress.MeetingAreas_selectedControlItems_length = MeetingAreas_selectedControlItems.value.length//check MeetingAreas_selectedControlItems length
            ...
            //---------------------------------set progress bar---------------------------------------------
            approveStatus.value = progress.approveStatus_ //progress
            if(progress.MeetingAreas_selectedControlItems_length > 0)
            {
              steps.value = [
                { status: 1, label: "สร้างรายการ" },
                { status: 2, label: "หัวหน้าอนุมัติ" },
            ...
            //---------------------------------get vrf approve_info---------------------------------------------
            try {
              loading.value = true
              await axios
                .get(process.env.VUE_APP_API_URL + '/get_vrf_approve_info', { params })
                .then(
                  (res) => {
                    // success callback
                    let obj = JSON.parse(JSON.stringify(res.data))
                    vrf_Existing.vrf_Existing_ApproveInfo = obj
                    setTimeout(() => {
                      table.isLoading = false
                      loading.value = false
                    }, 500)
            ...
            try {
              ...
              await axios
                ...
                .then(
                  ...
                  (res) => {
                    // error callback
                    console.log('vrf_urgent data:', res.data)
                  }
                )
                .finally(() => {
                  //
              ...
              await axios
                ...
                .then(
                  (res) => {
                    ...
                    vrf_Existing.vrf_Existing_det = obj
                    console.log(
                      'urgent vrf_Existing.vrf_Existing_det: ',
                      vrf_Existing.vrf_Existing_det
                    )
                    //-----------urgent vrf
              ...
              await axios
                ...
                .then(
                  (res) => {
                    ...
                    vrf_Existing.vrf_Existing_det = obj
                    console.log(
                      'edit_urgentcase_vrf vrf_Existing.vrf_Existing_det: ',
                      vrf_Existing.vrf_Existing_det
                    )
                    //-----------urgent vrf
                    // vrf_urgent.vrf_Existing_det = obj
    ...
    const setFromDateAdd = () => {
      if (NewVrf.date_from) {
        ...
        if (dateLimit > lastDayOfMonth) {
          alert("วันสิ้นสุดต้องไม่เกินวันที่สุดท้ายของเดือน")
          NewVrf.date_to = lastDayOfMonth // ใช้ format_date
        }
        else {
          NewVrf.date_to = dateLimit
        }
        NewVrf.date_to = dateLimit <= lastDayOfMonth ? dateLimit : lastDayOfMonth;
      }
    }
    // const dateFrom = ref(null); // ใช้ ref สำหรับ reactive data
    // const dateTo = ref(null);
    const setToDateAdd = () => {

      console.log('setToDateAdd called', NewVrf.date_from, NewVrf.date_to); // ตรวจสอบค่า

      if (NewVrf.date_from && NewVrf.date_to) {
        // dateFrom และ dateTo เป็น Date object อยู่แล้ว
        const from = NewVrf.date_from;
        const to = NewVrf.date_to;

        if (to.getTime() < from.getTime()) {
          alert('วันสิ้นสุดต้องมากกว่าวันที่เริ่มต้น');
          NewVrf.date_to = null; // หรือ ""
          return;
        }

        const lastDayOfMonth = new Date(from.getFullYear(), from.getMonth() + 1, 0);
        if (to.getTime() > lastDayOfMonth.getTime()) {
          alert('วันที่สิ้นสุดต้องไม่เกินวันสุดท้ายของเดือน');
          NewVrf.date_to = null; // หรือ ""
        }
      }
    };
    ...
    const AddUrgentcase = async () => {
      ...
      for (let index in vrf_Existing.vrf_Existing_det) {
        if (vrf_Existing.vrf_Existing_det[index].remark_urgentcase) {
          object_det.push({
            ...
            prefix: vrf_Existing.vrf_Existing_det[index].ddlprefix,
      ...
      for (let index in vrf_urgent.vrf_Existing_det) {
        object_det.push({
          ...
          prefix: vrf_urgent.vrf_Existing_det[index].ddlprefix,
...
@import '../assets/css/style_progress.css';
