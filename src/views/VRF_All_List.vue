                    ...
                    <div class="col-sm-2 ps-0">
                      <VueMultiselect name="area" id="area" :options="data_ddl.area"
                        class="form-select form-select-sm p-0" label="meeting_area" :style="{
                          width: '15rem',
                          height: '0.5rem'
                        }" v-model="AdvSearch.area_id" :select-label="null" :allow-empty="true" :close-on-select="true" track-by="id"
                        placeholder="เลือก" :deselectLabel="null"></VueMultiselect>
                    </div>
                  </div>
                  <div class="row p-2">
              ...
              <div class="modal-body modalcustom">
                <div class="container modalcustom">
                  ...
                  <div class="row p-2">
                    ...
                    <div class="col-md-2">
                      <input type="text" id="contactor" class="form-control" style="width: 15rem"
                        v-model="vrf_Existing.requestor" :disabled="!checkstatus_send_to_approve" />
                    </div>
                    ...
                    <div class="col-md-2">
                      <input type="text" id="contactor" class="form-control" style="width: 15rem"
                        v-model="vrf_Existing.department" :disabled="!checkstatus_send_to_approve" />
                  ...
                  <div class="row p-2">
                    ...
                    <div class="col-md-2">
                      <input type="text" id="contactor" class="form-control" style="width: 15rem"
                        v-model="vrf_Existing.navigator" :disabled="!checkstatus_send_to_approve" />
                    </div>
                ...
                <div class="container">
                  ...
                  <div class="row p-2">
                    <div class="col-sm-12 align-center ps-0">
                      <h3>คุณต้องการยกเลิกรายการเข้าพื้นที่?</h3>
                    </div>
                  </div>
                  <div class="row p-2">
  ...
  setup() {
    ...
    const setReject_vrf = async () => {
      const data = {
        ...
        cancel_reason: reject_vrf.reject_reason,
        CancelBy: localStorage.getItem('user_id')
      }
      //------------------add header----------------------
      try {
        await axios
          .post(process.env.VUE_APP_API_URL + '/set_su_cancel_vrf', data)
          .then(
            (res) => {
              // success callback
              console.log('set_su_cancel_vrf res.data: ', res.data)
            },
            (res) => {
              // error callback
    ...
    const ClosemyModalNew_ = () => {
      ...
      (NewVrf.reason = ''),
        ...
        (vrf_Existing.vrf_Existing_ApproveInfo = []),
        (vrf_Existing.errors = {}),
        //------------------------------------------
        (vrf_urgent.no = ''),
        (vrf_urgent.id = ''),
      ...
      await axios
        ...
        .then(
          (res) => {
            ...
            console.log('data_ddl.userlist: ', data_ddl.userlist);
          },
          (res) => {
            // error callback
            // ActitySelectd.branchtobranch
    ...
    const table = reactive({
      ...
      columns: [
        ...
        },
        {
          ...
          field: 'urgentcase_vrf',
          width: '15%',
          sortable: true,
          display: function (row) {
            return `<div  data-id="` +
              row.id +
              `" class="is-rows-el editvrf" data-bs-target="#ModalEditvrf" data-bs-toggle="modal" style="text-align: center; cursor: pointer;">${row.urgentcase_vrf !== null ? 'เคสด่วน' : ''}</div>`
          }
        },
        {
          // label: 'เหตุผลในการตีกลับ',
        ...
        },
        {
          ...
          height: '1%',
          display: function (row) {
            let button_cancel_status = row.approve_status_ === 5 ? '' : 'disabled'
            return (
              ...
              + '<button type="button" data-id="' +
              row.id +
              '" class="btn btn-danger is-rows-el rejectvrf" style="margin-top: 0.2rem; width: 6rem; height:2rem; cursor: pointer;" data-bs-target="#ModalRejectVRF"  data-bs-toggle="modal" ' + button_cancel_status + '>ยกเลิก</button>'
              +
              ' </div>'
            )
          }
              ...
              await axios
                ...
                .then(
                  (res) => {
                    ...
                    console.log('vrf_Existing_det: ', vrf_Existing.vrf_Existing_det)
                    setTimeout(() => {
                      table.isLoading = false
                      loading.value = false
                    }, 500)
