    ...
    <div class="row p-0" style="width: 100%">
      <div class="col-2 ps-4" style="text-align: left">
        <button type="button" class="btn btn-danger" style="width: 5rem; height: 2rem"
          @click="update_vrfstatus_all('cancel')">
          ยกเลิก
        </button>
        <!--&nbsp;
                  ...
                  <div class="row p-2">
                    ...
                    <div class="col-sm-2 ps-0">
                      <VueMultiselect name="requestor_dept" id="requestor_dept" :options="data_ddl.dept"
                        ...
                        :close-on-select="true" track-by="id" placeholder="เลือก" :deselectLabel="null">
                      </VueMultiselect>
                    </div>
                  </div>
                </div>
          ...
          <div class="modal-dialog modalcustom">
            <div class="modal-content p-0 m-0">
              ...
              <div class="modal-body">
                          ...
                          <div style="display: flex; align-items: center; justify-content: center;">
                            ...
                            <div style="display: flex; flex-direction: column; align-items: center;">
                              <select class="form-select form-select-sm" v-model="data.ddlprefix"
                                style="width: 7rem; margin-right: 0.5rem; display: inline-block;"
                                @change="validateInput('prefix', index, 'กรุณาคำนำหน้า')">
                                <option v-for="option in data_ddl.prefix" :value="option.id" :key="option.id">
                                  {{ option.prefix }}
                                </option>
                              </select>
                              <p class="error-message" style="margin-top: 2px; text-align: center;"
                                v-if="data.errors && data.errors.ddlprefix">
                                {{ data.errors.ddlprefix }}
                              </p>
                            </div>
                            ...
                            <div style="display: flex; flex-direction: column; align-items: center;">
                              <input type="text" class="form-control" style="width: 17rem; display: inline-block"
                                v-model="data.tbFullName" @input="complete_word_FullName('tbFullName', index)"
                  ...
                  <div class="col-md-10 text-left h-auto">
                    <ul class="selected-items-list w-100 border border-gray rounded-lg">
                      <li v-for="item in MeetingAreas_selectedItems" :key="item.id" class="selected-item">
                        {{ item.name }}
                        <button type="button" @click="removeSelectedItem(item)" class="remove-button">
                          &times;
                        </button>
                      </li>
                    </ul>
                ...
                <div class="row p-2 pe-2 me-2">
                  ...
                  <div class="col-md-10 text-left">
                    ...
                    <MeetingArea :items="MeetingArea_items" :categoryLabels="categoryAreas"
                      ...
                      @remove:item="removeSelectedItem" @update:isAllSelected="isAllSelected = $event" class="p-0 m-0"
                      style="width: auto" :controlLabel="'พื้นที่ทั่วไป'" :showSelectAll="false" />
                  </div>
                </div>
                  ...
                  <div class="col-md-10 text-left h-auto">
                    <ul class="selected-items-list w-100 border border-gray rounded-lg">
                      <li v-for="item in MeetingAreas_selectedControlItems" :key="item.id" class="selected-item">
                        {{ item.name }}
                        <button type="button" @click="removeSelectedControlItem(item)" class="remove-button">
                          &times;
                        </button>
                      </li>
                    </ul>
                ...
                <div class="row p-2 pe-2 me-2">
                  <div class="col-md-2 text-right">จากวันที่:</div>
                  ...
                  <div class="col-md-2 text-right">ถึงวันที่:</div>
                  <div class="col-md-2 text-left">
                ...
                <div class="row p-2 pe-2 me-2">
                  <div class="col-md-2 text-right">ชื่อแม่แบบ:</div>
                  <div class="col-md-2 text-left">
                    <input type="text" id="template_name" class="form-control" style="width: 100%"
                      v-model="NewVrf.template_name" />
                    <p v-if="VRF_error.template_name && !NewVrf.template_name" class="error-message">
                      กรุณากรอกข้อมูล
                    </p>
                  </div>
                  <div class="col-md-2 text-right">เหตุผลในการเข้าพบ:</div>
                  <div class="col-md-2 text-left">
                  ...
                  <div class="col-md-2 text-right">ชื่อบริษัทผู้มาติดต่อ:</div>
                  <div class="col-md-2 text-left">
                ...
                <div class="row p-2 pe-2 me-2">
                  <div class="col-md-2 text-right">ผู้ร้องขอ:</div>
                  <div class="col-md-2 text-left">
                    <VueMultiselect name="requestor" id="requestor" :options="data_ddl.userlist"
                      class="form-select form-select-sm p-0" label="first_name" :style="{
                        width: '15rem',
                      ...
                      :value="user_id" track-by="user_id" placeholder="เลือก" :deselectLabel="null"></VueMultiselect>
                    <p v-if="VRF_error.requestor && !NewVrf.requestor" class="error-message">
                      กรุณาเลือกข้อมูล
                    </p>
                  </div>
                  <div class="col-md-2 text-right">ตำแหน่งผู้ร้องขอ:</div>
                  <div class="col-md-2 text-left">
                    <VueMultiselect name="requestor_position" id="requestor_position" :options="data_ddl.position"
                      ...
                      :close-on-select="true" :value="NewVrf.requestor_position.id" track-by="id" placeholder="เลือก"
                      :deselectLabel="null"></VueMultiselect>
                    <p v-if="
                  ...
                  <div class="col-md-2 text-right">แผนกผู้ร้องขอ:</div>
                  <div class="col-md-2 text-left">
                    <VueMultiselect name="requestor_dept" id="requestor_dept" :options="data_ddl.dept"
                      class="form-select form-select-sm p-0" label="department" :style="{
                        width: '15rem',
                      ...
                      :close-on-select="true" :value="NewVrf.requestor_dept.id" track-by="id" placeholder="เลือก"
                      :deselectLabel="null"></VueMultiselect>
                ...
                <div class="row p-2 pe-2 me-2">
                  <div class="col-md-2 text-right">เบอร์โทรผู้ร้องขอ:</div>
                  <div class="col-md-2 text-left">
                  ...
                  <div class="col-md-2 text-right">ชื่อผู้นำพา:</div>
                  <div class="col-md-2 text-left">
                  ...
                  <div class="row ms-4 ps-4 pe-3 me-3 modalcustom">
                    <table class="table table-hover modalcustomtb">
                      <thead>
                        <tr>
                          ...
                          <th scope="col" class="colwidth10">
                            ดำเนินการ
                            <span @click.prevent="
                              addExistingItem(templete_vrf_Existing.id)
                      ...
                      <tbody>
                        <tr v-for="(
data, index
                          ) in templete_vrf_Existing.templete_vrf_Existing_det" :key="index">
                          <td scope="col" class="colwidth25" style="white-space: nowrap; text-align: center">
                            <div style="display: flex; align-items: center; justify-content: center;">
                              <!-- Dropdown Select -->
                              <div style="display: flex; flex-direction: column; align-items: center;">
                                <select class="form-select form-select-sm"
                                  v-model="templete_vrf_Existing.templete_vrf_Existing_det[index].prefix_id"
                                ...
                                <p class="error-message" style="margin-top: 2px; text-align: center;"
                                  v-if="data.errors && data.errors.prefix_id">
                                  {{ data.errors.prefix_id }}
                                </p>
                              </div>
                          ...
                          <td scope="col" class="colwidth10">
                            <select class="form-select form-select-sm" v-model="templete_vrf_Existing.templete_vrf_Existing_det[
                              index
                            ].vehicle_brand_id
                              " @change="
                                edit_validateInput(
                          ...
                          <td scope="col" class="colwidth10">
                            <input type="text" class="form-control" style="width: 10rem; display: inline-block" v-model="templete_vrf_Existing.templete_vrf_Existing_det[
                              index
                          ...
                          <td scope="col" class="colwidth25" style="text-align: center">
                            <input type="text" class="form-control" style="width: 20rem; display: inline-block" v-model="templete_vrf_Existing.templete_vrf_Existing_det[
                              index
                            ].card_no
                              " @blur="
                                edit_validateInput(
                  ...
                  <div class="row p-2">
                    ...
                    <div class="col">
                      ...
                      <p v-if="
                        VRF_error.date_from &&
                        !templete_vrf_Existing.date_from
                      " class="error-message">
                        กรุณาเลือกข้อมูล
                      </p>
                    </div>
                    ...
                    <div class="col">
                      ...
                      <p v-if="
                        VRF_error.date_to && !templete_vrf_Existing.date_to
                      " class="error-message">
                        กรุณาเลือกข้อมูล
                      </p>
                    </div>
                  ...
                  <div class="row p-2">
                    ...
                    <div class="col">
                      ...
                      <p v-if="VRF_error.reason && !templete_vrf_Existing.reason" class="error-message">
                        กรุณากรอกข้อมูล
                      </p>
                    </div>
  ...
  setup() {
    ...
    const fetchMeetingControlAreas = async () => {
      try {
        ...
        // console.log('initial MeetingControlArea_items.value: ', MeetingControlArea_items.value)
      } catch (error) {
        console.log(error)
      }
    }
    ...
    const updateSelectedControlItems = (items) => {
      ...
      // อัปเดต MeetingAreas_selectedControlItems
      MeetingAreas_selectedControlItems.value = items;

      // อัปเดตสถานะ "Select All" และหมวดหมู่
      checkAllControlSelected();
    };
    const checkAllSelected = () => {
      // ตรวจสอบสถานะของ Select All
      isAllSelected.value = Object.keys(MeetingArea_items.value).every((categoryKey) =>
        MeetingArea_items.value[categoryKey].every((item) => item.selected)
      );

      // ตรวจสอบสถานะของแต่ละหมวดหมู่
      for (let categoryKey in MeetingArea_items.value) {
        const category = MeetingArea_items.value[categoryKey];
        const allSelected = category.every((item) => item.selected);
        console.log(`Category ${categoryKey} is all selected:`, allSelected);
      }
    };
    const checkAllControlSelected = () => {
      // ตรวจสอบสถานะ "Select All" สำหรับ MeetingControlArea_items
      isAllSelected.value = Object.keys(MeetingControlArea_items.value).every(
        (categoryKey) =>
          MeetingControlArea_items.value[categoryKey].every((item) => item.selected)
      );

      // ตรวจสอบสถานะของแต่ละหมวดหมู่
      for (let categoryKey in MeetingControlArea_items.value) {
        const category = MeetingControlArea_items.value[categoryKey];
        const allSelected = category.every((item) => item.selected);
        //console.log(`Control Category ${categoryKey} is all selected:`, allSelected);
      }
    };
    const removeSelectedItem = (item) => {
      console.log('Removing item:', item);

      // ลบรายการออกจาก MeetingAreas_selectedItems
      MeetingAreas_selectedItems.value = MeetingAreas_selectedItems.value.filter(
        (i) => i.id !== item.id
      );

      console.log('before MeetingArea_items.value:', JSON.stringify(MeetingArea_items.value));

      for (let categoryKey in MeetingArea_items.value) {
        const category = MeetingArea_items.value[categoryKey];
        const targetIndex = category.findIndex((i) => i.id === item.id);

        if (targetIndex !== -1) {
          console.log('targetItem found:', category[targetIndex]);

          // ตั้งค่าเป็น unselected
          category[targetIndex] = { ...category[targetIndex], selected: false };

          // **บังคับ Vue ให้ตรวจจับการเปลี่ยนแปลง**
          MeetingArea_items.value[categoryKey] = [...category];
          console.log('after MeetingArea_items.value:', JSON.stringify(MeetingArea_items.value));
      ...
      // ตรวจสอบสถานะ "Select All" และหมวดหมู่ใหม่
      checkAllSelected();
    };
    const removeSelectedControlItem = (item) => {
      console.log('Removing control item:', item);

      if (item.is_area_group === false) {
        // ลบรายการออกจาก MeetingAreas_selectedControlItems
        MeetingAreas_selectedControlItems.value =
          MeetingAreas_selectedControlItems.value.filter((i) => i.id !== item.id);

        // อัปเดตสถานะ checkbox ใน MeetingControlArea_items ให้ unselected
        for (let key in MeetingControlArea_items.value) {
          const category = MeetingControlArea_items.value[key];
          const targetIndex = category.findIndex((i) => i.id === item.id);
          if (targetIndex !== -1) {
            console.log('Found targetItem in category:', key, category[targetIndex]);
            // ตั้งค่า selected เป็น false
            category[targetIndex] = { ...category[targetIndex], selected: false };
            // บังคับให้ Vue อัปเดตหมวดหมู่ใหม่
            MeetingControlArea_items.value[key] = [...category];
            break;
          }
        }
        //console.log('after MeetingControlArea_items.value:', JSON.stringify(MeetingControlArea_items.value));  
      } else if (item.is_area_group === true) {
        // กรณีลบทั้ง category
        const categoryName = item.name.replace(' ทั้งหมด', ''); // แปลงชื่อ category กลับจาก "ชื่อ ทั้งหมด"
        console.log('Removing all items in category:', categoryName);
        if (MeetingControlArea_items.value[categoryName]) {
          // ตั้งค่า selected ของทุกไอเท็มใน category เป็น false
          MeetingControlArea_items.value[categoryName].forEach((i) => {
            i.selected = false;

          });
          // อัปเดต Vue ด้วย array ใหม่
          MeetingControlArea_items.value[categoryName] = [...MeetingControlArea_items.value[categoryName]];
          // ลบไอเท็มทั้งหมดใน category จาก MeetingAreas_selectedControlItems
          MeetingAreas_selectedControlItems.value = MeetingAreas_selectedControlItems.value.filter(
            (i) => i.name !== item.name && !MeetingControlArea_items.value[categoryName].some((cItem) => cItem.id === i.id)
          );

        }
      }
      // ตรวจสอบสถานะ "Select All" ใหม่
      checkAllControlSelected();
    };
    ...
    const confirmDialog = async () => {
      if (function_selected.value === 'update_vrfstatus_all') {
        ...
        try {
          ...
          loading.value = false
          await axios
            .get(process.env.VUE_APP_API_URL + '/update_vrfstatus_all', {
              params
            })
            .then(
              (res) => {
      ...
      if (function_selected.value === 'update_vrfstatus') {
        ...
        try {
          loading.value = true
          await axios
            .get(process.env.VUE_APP_API_URL + '/update_vrfstatus', { params })
            .then(
              (res) => {
                // success callback
                let obj = JSON.parse(JSON.stringify(res.data))
    ...
    const myRequest = async (keyword) => {
      ...
      try {
        const res = await axios.get(
          process.env.VUE_APP_API_URL + '/get_templete_vrf_list',
          { params }
        )
        ...
        await axios
          .get(process.env.VUE_APP_API_URL + '/get_vehicle_brand')
          .then(
            (res) => {
        ...
        await axios
          .get(process.env.VUE_APP_API_URL + '/get_vehicle_color')
          .then(
            (res) => {
      ...
      columns: [
        {
          ...
          display: function (row) {
            ...
            return `<div data-id="${row.id}" class="is-rows-el editvrf cursor-pointer" style="margin-top: 0.2rem; width: 5rem; height:2rem;  cursor: pointer;" data-bs-target="#ModalEditvrf" data-bs-toggle="modal">${row.no}</div>`
        ...
        },
        {
          ...
          display: function (row) {
            return `
            <div data-id="${row.id
              }" class="is-rows-el editvrf cursor-pointer" style="margin-top: 0.2rem; width: 5rem; height:2rem;  cursor: pointer;" data-bs-target="#ModalEditvrf" data-bs-toggle="modal">${row.date_from ? dateTime(row.date_from) : ''
              }</div>
            `
          }
        },
        {
          ...
          display: function (row) {
            return `<div data-id="${row.id
              }" class="is-rows-el editvrf cursor-pointer" style="margin-top: 0.2rem; width: 5rem; height:2rem;  cursor: pointer;" data-bs-target="#ModalEditvrf" data-bs-toggle="modal">${row.date_to ? dateTime(row.date_to) : ''
              }</div>
            `
          }
        },
        {
          ...
          display: function (row) {
            return `
            <div data-id="${row.id}" class="is-rows-el editvrf cursor-pointer" style="text-align: left; white-space: normal !important; margin-top: 0.2rem; width: 15rem; height:2rem;  cursor: pointer;" data-bs-target="#ModalEditvrf" data-bs-toggle="modal">${row.contactor}</div>            
            `
          }
        },
        {
          ...
          display: function (row) {
            return `
            <div data-id="${row.id}" class="is-rows-el editvrf cursor-pointer" style="text-align: left; white-space: normal !important; margin-top: 0.2rem; width: 15rem; height:2rem;  cursor: pointer;" data-bs-target="#ModalEditvrf" data-bs-toggle="modal">${row.reason}</div>            
            `
          }
        },
        {
          ...
          display: function (row) {
            return `
            <div data-id="${row.id
              }" class="is-rows-el editvrf cursor-pointer" style="text-align: left; cursor: pointer;" data-bs-target="#ModalEditvrf" data-bs-toggle="modal">${row.area_names ? row.area_names.join(', ') : ''
              }</div>
        ...
        if (element.classList.contains('editvrf')) {
          element.addEventListener('click', async function () {
            ...
            try {
              loading.value = true
              await axios
                .get(process.env.VUE_APP_API_URL + '/get_templete_vrf', {
                  params
                })
                .then(
                  (res) => {
            ...
            try {
              loading.value = true
              await axios
                .get(process.env.VUE_APP_API_URL + '/get_templete_vrf_det', {
                  params
                })
                .then(
                  (res) => {
    ...
    const AdvSearch_ = async () => {
      ...
      await axios
        .get(process.env.VUE_APP_API_URL + '/get_search_vrf_templete', {
          params
        })
        .then(
          (res) => {
    ...
    const addManualVRF = async () => {
      ...
      try {
        await axios
          .post(
            process.env.VUE_APP_API_URL + '/set_manual_add_vrf_template',
            json_formData
          )
          .then(
            (res) => {
      ...
      try {
        await axios
          .post(
            process.env.VUE_APP_API_URL + '/set_manual_add_vrf_template_det',
            json_object_det
          )
          .then(
            (res) => {
            ...
            (res) => {
              console.error(
                'set_manual_add_vrf_template_det error callback',
                res.data.message
              )
    ...
    const validateInput = (field, index, errorMessage) => {
      const item = rowData.value[index];

      if (field === 'tbFullName') {
        const isDuplicate = rowData.value.some(
          (otherItem, idx) =>
            idx !== index && otherItem.tbFullName === item.tbFullName
        );
        ...
        if (isDuplicate) {
          ...
          if (item.errors && item.errors[field]) {
            const { [field]: removed, ...rest } = item.errors;
            item.errors = { ...rest };
          }
      ...
      else if (field === 'ddlprefix') {
        if (!item.ddlprefix) {
          item.errors = {
            ...item.errors,
            [field]: 'กรุณาเลือกคำนำหน้า'
          };
          validateInputAll.value = true;
        } else {
          if (item.errors && item.errors[field]) {
        ...
        if (item.tbVehicle_Registration && item.tbVehicle_Registration.length > 10) {
          item.errors = {
            ...item.errors,
    ...
    const edit_validateInput = (field, index, errorMessage) => {
      const item = templete_vrf_Existing.templete_vrf_Existing_det[index];

      if (field === 'fullname') {
        const isDuplicate = templete_vrf_Existing.templete_vrf_Existing_det.some(
          (otherItem, idx) =>
            idx !== index && otherItem.fullname === item.fullname
        );
        ...
        if (item.vehicle_registration && item.vehicle_registration.length > 10) {
          item.errors = {
            ...item.errors,
            [field]: 'ห้ามใส่ข้อมูลเกิน 10 ตัวอักษร'
          };
    ...
    const editVRF = async () => {
      ...
      try {
        ...
        await axios
          .post(
            process.env.VUE_APP_API_URL + '/set_update_vrf_template',
            json_formData
          )
          .then(
            (res) => {
      ...
      for (let index in templete_vrf_Existing.templete_vrf_Existing_det) {
        object_det.push({
          ...
          prefix:
            templete_vrf_Existing.templete_vrf_Existing_det[index].prefix_id,
          fullname:
            templete_vrf_Existing.templete_vrf_Existing_det[index].fullname,
          vrf_id: templete_vrf_Existing.id,
          vehicle_brand_id:
            templete_vrf_Existing.templete_vrf_Existing_det[index]
              .vehicle_brand_id,
          vehicle_color_id:
            templete_vrf_Existing.templete_vrf_Existing_det[index]
              .vehicle_color_id,
          vehicle_registration:
            templete_vrf_Existing.templete_vrf_Existing_det[index]
              .vehicle_registration,
          card_no:
            templete_vrf_Existing.templete_vrf_Existing_det[index].card_no,
          user_id: user_id.value
        })
      }
      var json_object_det = JSON.stringify(object_det)
      ...
      try {
        await axios
          .post(
            process.env.VUE_APP_API_URL + '/set_update_vrf_template_det',
            json_object_det
          )
          .then(
            (res) => {
    ...
    const setToDate = (type) => {
      ...
      if (type === 'date_from_edit') {
        ...
        templete_vrf_Existing.date_to = new Date(
          newDate.getFullYear(),
          newDate.getMonth() + 1,
          newDate.getDate() - 1
        )
